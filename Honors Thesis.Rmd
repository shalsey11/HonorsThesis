...
survey <- read_csv("Mental Health Among Collegiate Athletes1.csv")
glimpse(survey)
```
...
```{r categorizing-gpa}
survey1 <- survey1 %>%
  mutate(gpa_group = case_when(majorgpa >= 3.7 ~ "3.7 - 4.0",
                               majorgpa >= 3.4 ~ "3.4 - 3.7",
                               majorgpa >= 3.0 ~ "3.0 - 3.4",
                               majorgpa >= 2.5 ~ "2.5 - 3.0",
                               TRUE ~ "below 2.5"))
```
...
Below is a graph representing points of anxiety_total based on gpa group. This is  a little difficult to see. 
```{r graph}
ggplot(data = survey1, aes(x = gpa_group, y = anxiety_total)) +
  geom_point()
...
ggplot(data = survey1, aes(y = anxiety_total, color = gpa_group)) + geom_boxplot() + labs(y = "Total Anxiety Score", title = "Distribution of Total Anxiety Score", subtitle = "By GPA", color = "GPA") + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
```
```{r plot5}
ggplot(data = survey1, aes(x = anxiety_total)) +
  geom_density()
