...
survey1 %>%
  ...
  labs(
    ...
    y = NULL,
    title = 'Prevelance of Anxiety', subtitle = 'By Athletic Distinction'
    ) +
...
survey1 %>%
  ...
  labs(
    ...
    fill = "Anxiety Prevelance", 
    y = NULL,
    title = 'Prevelance of Anxiety', subtitle = 'By Academic Department'
...
survey1 %>%
  filter(gpa_group == "below 2.6" | gpa_group == "2.6 - 3.0" | gpa_group == "3.0 - 3.4" | gpa_group == "3.4 - 3.7" | gpa_group == "3.7 - 4.0") %>%
...
survey1 %>%
  ...
  labs(
    x = "GPA", 
    y = "Anxiety Prevelance",
    title = "GPA vs. Anxiety Prevelance"
    ) +
...
```{r confusion-matrix-with-anxiety_fit1}
pred_train1 <- predict(anxiety_fit, survey1_train, type = "response")
#Calculate predicted 0/1 values for the  injury variable using a threshold of 0.5:
pred_num1 <- ifelse(pred_train1 > 0.5, 1, 0)
pred_num1 <- factor(pred_num1, levels = c(0,1))
confusionMatrix(pred_num1, survey1_train$anxiety_total_level)
```

```{r ROC-curve-with-anxiety_fit1}
anxiety_pred1 <- as_tibble(pred_train)
