...
survey1$gpa_group2 = factor(survey1$gpa_group2, levels=c("below 2.5", "2.5-3.0", "3.0-3.4", "3.4-3.7", "3.7-4.0"))
```

```{r boxplot-anxiety_total-gpa_group}
ggplot(data = survey1, aes(x = gpa_group2, y = anxiety_total, color = gpa_group2)) + geom_boxplot() + labs(x = "GPA Range", y = "Total Anxiety Score", title = "Distribution of Total Anxiety Score", subtitle = "By GPA", color = "GPA Range")
...
set.seed(1234)
survey2_split <- initial_split(survey2, prop = 0.75)
survey2_train <- training(survey2_split)
survey2_test <- testing(survey2_split)
```

```{r refactoring}
survey1 <- survey1 %>%
  mutate(type = if_else(pop == "Vic", "1", "0"),
         type = as.factor(type))
```

```{r logisitic-regression}
anxiety_fit <- glm(anxiety_total_level ~ cumgpa + major_group + athlete + Year, data = survey2, family = "binomial")
